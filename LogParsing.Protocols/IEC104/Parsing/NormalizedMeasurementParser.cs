...
{
    /// <summary>
    /// 提供对 IEC 60870-5-104 归一化遥测值（Normalized Measurement Value）的解析功能。
    /// </summary>
    /// <remarks>
    /// <para>
    /// 支持的标准类型包括：
    /// <see cref="IEC104TypeId.M_ME_NA_1"/>（无时标归一化遥测）和
    /// <see cref="IEC104TypeId.M_ME_TB_1"/>（带 CP56Time2a 时标的归一化遥测）。
    /// </para>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// 每个遥测对象由 3 字节 IOA + 2 字节归一化值（小端序）+ 1 字节品质描述（QDS）组成。
    /// 归一化值为 16 位有符号整数，范围 [-32768, 32767]，需由上层应用结合缩放因子转换为工程单位。
    /// 品质描述字节中的 OV（Overflow）位（bit 7）用于判断数据有效性：
    /// 若 OV = 0（即 <c>qds &amp; 0x80 == 0</c>），则视为有效。
    /// </para>
    /// </remarks>
    internal sealed class NormalizedMeasurementParser
    {
        /// <summary>
        /// 获取当前解析器支持的 IEC 104 类型标识符集合。
        /// </summary>
        /// <remarks>
        /// 该集合包含标准归一化遥测类型 <see cref="IEC104TypeId.M_ME_NA_1"/> 和
        /// 带时标扩展类型 <see cref="IEC104TypeId.M_ME_TB_1"/>。
        /// 若未来扩展支持其他归一化遥测类型，需同步更新此集合及解析逻辑。
        /// </remarks>
        private static readonly HashSet<IEC104TypeId> SupportedTypeIds =
            new() { IEC104TypeId.M_ME_NA_1, IEC104TypeId.M_ME_TB_1 };

        /// <summary>
        /// 解析归一化遥测 ASDU 载荷，并返回一组标准化的测量结果。
        /// </summary>
        /// <param name="typeId">
        ...
        /// 每个对象包含：3 字节 IOA + 2 字节归一化值（小端序）+ 1 字节品质描述（QDS）。
        /// </param>
        /// <param name="timestamp">
        /// 可选时间戳。对于 <see cref="IEC104TypeId.M_ME_TB_1"/>，
        /// 此值应来自已解析的 CP56Time2a 字段；
        /// 对于 <see cref="IEC104TypeId.M_ME_NA_1"/>，通常为帧接收时间或 <see langword="null"/>。
        /// </param>
            ...
            ushort causeOfTransmission)
        {
            if (!SupportedTypeIds.Contains(typeId))
                throw new NotSupportedException(
                    $"TypeId '{typeId}' is not supported by {nameof(NormalizedMeasurementParser)}.");
            ...
            for (int i = 0; i < numberOfObjects; i++)
            {
                ...
                short normalizedValue = (short)(payload[index] | (payload[index + 1] << 8));
                index += 2;
